...
%random walk, local minima method
theta = mean(theta_range);
ve2 = mean(ve2_range);
step_theta = theta_fat/100;
step_ve2 = ve2_fat/100;
SSE= (fun(theta,ve2,"vz"))^2+(fun(theta,ve2,"vt"))^2
while SSE > 1
    %step right
    SSE_prime = (fun(theta+step_theta,ve2,"vz"))^2+(fun(theta+step_theta,ve2,"vt"))^2;
    if SSE_prime < SSE
        theta = theta+step_theta %need to walk properly ya
